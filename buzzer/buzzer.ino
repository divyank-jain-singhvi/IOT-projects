...
void loop() {
  ...
  if (event.temperature > 20){ // temperature of a average mines is 40 degree 
    digitalWrite(buzz,HIGH);
    if (flag==0){
    sendingMail("Temperature is high , Danger Alert!!");
    }
    // delay(2000);
    // noTone(buzz);

  }  
  if (event.relative_humidity > 90){ // humidity of a average mines is 80% humidity 
    // digitalWrite(buzz,2000);
    if (flag==0){
      sendingMail("Condition of humidity is heigh");
    }
    // delay(2000);
    // noTone(buzz);
  }  
  if (analogRead(sensorPin) > 8000){ 
    // digitalWrite(buzz,2000);
    if (flag==0){
    sendingMail("Condition of gas is heigh");
      }
    // delay(2000);
    // noTone(buzz);
  }  
  ...
  if (analogRead(sensorPin) < 8000){ // humidity of a average mines is 50% humidity 
    // digitalWrite(buzz,LOW);
    // delay(2000);
    // noTone(buzz);
  }  
